...
- **자동 검색(`search.auto`)** — 출발/도착역 팝업, 날짜피커, 인원 선택은 신규 페이지 셀렉터 검증 필요. 현재는 사용자가 수동 입력하는 게 가장 안전.
- **자동 로그인 셀렉터** — `input#txtMember` / `input#txtPwd` / `button.btn_login` 가정. 페이지 변경 시 `korail.js` 의 `login()` 수정 필요.
